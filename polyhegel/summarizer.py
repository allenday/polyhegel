...
class StrategySummarizer:
    ...
    def __init__(self, model):
        ...
        """
        self.model = model
        self.agent = Agent(self.model, output_type=str, system_prompt=get_system_prompt("strategic", "summarizer"))

    async def summarize_results(
        self, trunk: Optional[StrategyChain], twigs: List[StrategyChain], cluster_metrics: Optional[dict] = None
    ) -> str:
        """
    ...
    def _build_summary_prompt(
        self, trunk: StrategyChain, twigs: List[StrategyChain], cluster_metrics: Optional[dict]
    ) -> str:
    ...
    def _format_strategy_brief(self, strategy: StrategyChain) -> str:
        ...
        risk_summary = (
            "High risk"
            if any("high" in r.lower() for step in strategy.strategy.steps for r in step.risks)
            else "Moderate risk"
        )
...
The primary strategy should be implemented with careful attention to resource requirements and risk mitigation."""
    ...
    async def generate_strategy_comparison(self, strategies: List[StrategyChain], focus_areas: List[str] = None) -> str:
        """
