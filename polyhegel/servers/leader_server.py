...
from polyhegel.security import (
    AgentRole, Permission, SecurityConfig, get_auth_manager,
    verify_api_key_auth, require_permission, require_role
)
from polyhegel.telemetry import (
    setup_telemetry_for_agent, A2AMonitoringMiddleware, TelemetryExporter
)
...
def create_leader_server(
    ...
    if enable_auth:
        ...
        async def get_agent_info(
            credentials = Depends(require_role(AgentRole.LEADER))
        ):
