...
class RecursiveRefinementEngine:
    ...
    async def refine_strategy(self,
                            ...
                            session_id: Optional[str] = None) -> RefinementSession:
        """
        ...
        try:
            ...
            logger.info(f"Refinement session {session_id} completed: "
    ...
    async def _execute_refinement_loop(self,
                                     session: RefinementSession,
        ...
        while session.current_generation < self.config.max_generations:
            ...
            feedback_analysis = await self.feedback_loop.analyze_performance(
                f"{session.session_id}_gen_{session.current_generation}",
                current_metrics
            )
            ...
            if (self.config.require_strategic_compliance and
                current_metrics.strategic_compliance_score >= self.config.strategic_compliance_target):
                session.completion_reason = f"Strategic compliance achieved ({current_metrics.strategic_compliance_score:.3f})"
                break
            ...
            try:
                improved_strategy = await self.strategy_improver.improve_strategy(
                    current_strategy,
                    feedback_analysis,
                    focus_areas=self.config.improvement_priorities or None
                )
                
                # Evaluate improved strategy
                improved_metrics = await self._evaluate_strategy(
                    improved_strategy, system_prompt, user_prompt
                )
                ...
                if improved_score > session.best_score:
                    ...
                    logger.info(f"Generation {session.current_generation} score: {improved_score:.2f} "
    ...
    async def _evaluate_strategy(self,
                               strategy: StrategyChain,
                               system_prompt: Optional[str],
                               user_prompt: Optional[str]) -> StrategicMetrics:
    ...
    async def _save_final_results(self, session: RefinementSession):
        ...
        session_data = {
            ...
            'results': {
                ...
                'total_improvement': session.best_score - session.performance_history[0].strategic_metrics.overall_score(),
    ...
    def get_global_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_refinement_time': (
                ...
                if self._total_strategies_refined > 0 else 0
            )
        }