...
class StrategyClusterer:
    ...
    def __init__(self, min_cluster_size: int = None, twig_threshold: float = None):
        """
    ...
    def _handle_insufficient_chains(self, chains: List[StrategyChain]) -> Dict[str, Any]:
        ...
        if chains:
            ...
            return {"trunk": chains[0], "twigs": [], "cluster_count": 1, "noise_count": 0}
        return {"trunk": None, "twigs": [], "cluster_count": 0, "noise_count": 0}
    ...
    def _analyze_clusters(
        ...
        logger.info(
            f"Clustering complete: {len(unique_labels)} clusters, "
    ...
    def _find_trunk(
        self, chains: List[StrategyChain], cluster_label: int, embeddings: np.ndarray
    ) -> Optional[StrategyChain]:
        """
