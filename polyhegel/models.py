...
class GenesisStrategy(BaseModel):
    ...
    alignment_score: Dict[str, float] = Field(description="How well this aligns with strategic domains and constraints")
...
class StrategicTheme(BaseModel):
    ...
    title: str = Field(description="Clear, concise title for the strategic theme", min_length=5, max_length=100)

    category: ThemeCategory = Field(description="Primary category this theme belongs to")

    description: str = Field(
        description="Detailed description of the strategic theme and its purpose", min_length=50, max_length=500
    )

    domain_alignment: Dict[str, float] = Field(
        description="Alignment scores with strategic domains (2.1, 2.2, 2.3) on 1-5 scale", default_factory=dict
    )

    priority_level: Literal["critical", "high", "medium", "low"] = Field(
        default="medium", description="Priority level for theme implementation"
    )
...
class StrategyEvaluationResponse(BaseModel):
    ...
    preferred_strategy_index: int = Field(description="Index of preferred strategy (1 or 2)", ge=1, le=2)
    ...
    coherence_comparison: Dict[str, float] = Field(
        description="Coherence scores for both strategies (1-10 scale)", default_factory=dict
    )

    feasibility_comparison: Dict[str, float] = Field(
        description="Feasibility scores for both strategies (1-10 scale)", default_factory=dict
    )

    risk_management_comparison: Dict[str, float] = Field(
        description="Risk management scores for both strategies (1-10 scale)", default_factory=dict
    )
...
class StrategyAnalysisResponse(BaseModel):
    ...
    overall_score: float = Field(description="Overall strategy effectiveness score (1-10 scale)", ge=1.0, le=10.0)

    coherence_score: float = Field(description="Logical flow and consistency score (1-10 scale)", ge=1.0, le=10.0)

    feasibility_score: float = Field(description="Implementation feasibility score (1-10 scale)", ge=1.0, le=10.0)

    risk_management_score: float = Field(
        description="Risk identification and mitigation score (1-10 scale)", ge=1.0, le=10.0
    )
