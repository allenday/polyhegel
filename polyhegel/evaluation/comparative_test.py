...
class ComparativeTestFramework:
    ...
    async def run_comparative_test(self,
                                 ...
                                 output_dir: Optional[Path] = None) -> ComparisonResult:
        """
        ...
        try:
            ...
            comparative_analysis.update(self._perform_statistical_analysis(
    ...
    async def _run_single_test(self,
                             ...
                             iteration: int) -> StrategicMetrics:
        ...
        try:
            ...
            metrics = self.metrics_collector.collect_metrics(
                results, execution_time, memory_usage
            )
            
            logger.info(f"{selection_method} iteration {iteration} completed: "
    ...
    def _perform_statistical_analysis(self,
                                    clustering_metrics: List[StrategicMetrics],
                                    tournament_metrics: List[StrategicMetrics]) -> Dict[str, Any]:
        ...
        for metric_name in clustering_scores.keys():
            ...
            analysis['effect_sizes'][metric_name] = {
                ...
                'winner': 'tournament' if t_mean > c_mean else 'clustering' if metric_name != 'execution_time' else ('clustering' if t_mean > c_mean else 'tournament')
            }
        ...
        if len(clustering_scores['overall']) > 1 and statistics.mean(clustering_scores['overall']) > 0:
            clustering_cv = statistics.stdev(clustering_scores['overall']) / statistics.mean(clustering_scores['overall'])
        
        if len(tournament_scores['overall']) > 1 and statistics.mean(tournament_scores['overall']) > 0:
            tournament_cv = statistics.stdev(tournament_scores['overall']) / statistics.mean(tournament_scores['overall'])
        ...
        clustering_wins = sum(1 for metric in ['overall', 'coherence', 'feasibility', 'domain_alignment'] 
                            if analysis['effect_sizes'][metric]['winner'] == 'clustering')
        tournament_wins = sum(1 for metric in ['overall', 'coherence', 'feasibility', 'domain_alignment'] 
                            if analysis['effect_sizes'][metric]['winner'] == 'tournament')
        
        analysis['performance_comparison'] = {
            'clustering_wins': clustering_wins,
            'tournament_wins': tournament_wins,
            'overall_winner': 'clustering' if clustering_wins > tournament_wins else 'tournament' if tournament_wins > clustering_wins else 'tie',
    ...
    def run_automated_test_suite(self,
                               ...
                               output_dir: Optional[Path] = None) -> List[ComparisonResult]:
        """
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            try:
                result = asyncio.run(framework.run_comparative_test(
