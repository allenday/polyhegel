...
class StrategyTournament:
    ...
    async def run_tournament(self,
                           ...
                           results_file: Optional[str] = None) -> Tuple[StrategyChain, Dict]:
        """
        ...
        best_strategy_idx = await self._elimination_tournament(
            candidates, context, num_comparisons, tournament_results
        )
    ...
    async def _elimination_tournament(self,
                                    candidates: Dict[int, StrategyChain],
                                    context: str,
                                    num_comparisons: int,
                                    results: Dict) -> int:
        """
    ...
    async def run_round_robin_tournament(self,
                                       strategies: List[StrategyChain],
                                       context: str,
                                       num_comparisons: int = 1) -> Dict:
        """
        ...
        for i in range(len(strategies)):
            for j in range(i + 1, len(strategies)):
                ...
                comparison_results = []
                for comp_round in range(num_comparisons):
                    result = await self._compare_strategies(
                        strategy1, strategy2, context, comp_round
                    )
...
class TournamentRunner:
    ...
    async def find_best_strategy(self,
                                strategies: List[StrategyChain],
                                context: str,
        ...
        if tournament_type == "elimination":
            ...
            results = await self.tournament.run_round_robin_tournament(
                strategies, context, num_comparisons
            )
            if save_results and results_file:
    ...
    async def compare_technique_strategies(self,
                                         strategies: List[StrategyChain],
                                         context: str) -> Dict:
        """
