...
class TestFullIntegration:
    ...
    async def test_strategic_query_full_simulation(self):
        ...
        try:
            ...
            print("Using model: claude-3-haiku-20240307")
            print(f"Temperature settings: {temperature_counts}")
            print(f"System prompt length: {len(system_prompt)} chars")
            print(f"User prompt length: {len(user_prompt)} chars")

            results = await simulator.run_simulation(
                temperature_counts=temperature_counts, system_prompt=system_prompt, user_prompt=user_prompt
            )
            ...
            with open(summary_file, "w") as f:
                ...
                f.write("Model: claude-3-haiku-20240307\n")
            ...
            with open(report_file, "w") as f:
                ...
                f.write("**Model Used:** claude-3-haiku-20240307\n")
                ...
                f.write(
                    f"- **Web Tools:** {'✓ Available' if 'web_search_tool' in str(results) or 'web_fetch_tool' in str(results) else '? Not explicitly used'}\n"
                )
                f.write(
                    f"- **Git Tools:** {'✓ Available' if 'git_repo_to_md_tool' in str(results) else '? Not explicitly used'}\n\n"
                )
