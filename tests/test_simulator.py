...
class TestPolyhegelSimulatorIntegration:
        ...
        with patch.object(self.simulator.generator, 'generate_strategies', new_callable=AsyncMock) as mock_gen:
            with patch.object(self.simulator.embedder, 'embed_strategies') as mock_embed:
                with patch.object(self.simulator.clusterer, 'cluster_strategies') as mock_cluster:
                    with patch.object(self.simulator.summarizer, 'summarize_results', new_callable=AsyncMock) as mock_summary:
    ...
    async def test_hierarchical_mode_simulation(self):
        ...
        with patch.object(self.simulator, '_generate_hierarchical_strategies', new_callable=AsyncMock) as mock_hierarchical:
            with patch.object(self.simulator.embedder, 'embed_strategies') as mock_embed:
                with patch.object(self.simulator.clusterer, 'cluster_strategies') as mock_cluster:
                    with patch.object(self.simulator.summarizer, 'summarize_results', new_callable=AsyncMock) as mock_summary:
    ...
    async def test_hierarchical_strategy_generation(self):
        ...
        mock_themes = [
            StrategicTheme(
                ...
                clm_alignment={"2.1": 4.5, "2.2": 2.0, "2.3": 2.5},
        ...
        with patch('polyhegel.simulator.AgentCoordinator') as mock_coordinator_class:
            with patch('polyhegel.simulator.LeaderAgent') as mock_leader_class:
                with patch('polyhegel.simulator.FollowerAgent') as mock_follower_class:
