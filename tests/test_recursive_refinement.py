...
from polyhegel.refinement.feedback import ImprovementSuggestion, ImprovementCategory, FeedbackAnalysis
...
class TestPerformanceTracker:
    ...
    def test_record_performance_initial(self):
        ...
        metrics = self.tracker.record_performance(
            self.sample_strategy, strategic_metrics, generation=0, refinement_id="test_refinement"
        )
    ...
    def test_record_performance_improvement(self):
        ...
        self.tracker.record_performance(
            self.sample_strategy, initial_metrics, generation=0, refinement_id="test_refinement"
        )

        # Record improved performance
        improved_metrics = StrategicMetrics(coherence_score=8.0, feasibility_score=8.0)
        metrics = self.tracker.record_performance(
            self.sample_strategy, improved_metrics, generation=1, refinement_id="test_refinement"
        )
...
class TestFeedbackLoop:
    ...
    async def test_analyze_performance_basic(self):
        ...
        with patch.object(self.tracker, "get_all_metrics", return_value=[self.sample_metrics]):
            analysis = await self.feedback_loop.analyze_performance("test_strategy", self.sample_metrics)
...
class TestStrategyImprover:
    ...
    async def test_improve_strategy_rules(self):
        ...
        improved_strategy = await self.improver._generate_improved_strategy_rules(
            self.sample_strategy, self.sample_analysis.improvement_suggestions
        )
    ...
    def test_build_improvement_context(self):
        ...
        context = self.improver._build_improvement_context(
            self.sample_strategy, self.sample_analysis, self.sample_analysis.improvement_suggestions
        )
    ...
    async def test_evaluate_strategy_mock(self):
        ...
        with patch("polyhegel.refinement.recursive.PolyhegelSimulator", return_value=mock_simulator):
            metrics = await self.engine._evaluate_strategy(
                self.sample_strategy, "Test system prompt", "Test user prompt"
            )
...
class TestRefinementConfiguration:
    ...
    def test_default_configuration(self):
        ...
        assert config.require_strategic_compliance

    def test_custom_configuration(self):
        """Test custom configuration"""
        config = RefinementConfiguration(max_generations=5, quality_target=9.0, model_name="test-model")
...
async def test_integration_refinement_workflow():
    ...
    # Create configuration
    config = RefinementConfiguration(max_generations=2, output_directory=Path(temp_dir))  # Keep small for testing
    ...
    strategy_chain = StrategyChain(strategy=strategy, source_sample=1, temperature=0.8)
    ...
    with patch("polyhegel.refinement.recursive.PolyhegelSimulator", return_value=mock_simulator):
        ...
        session = await engine.refine_strategy(
            strategy_chain, "Integration test system prompt", "Integration test user prompt"
        )
