...
class TestStrategyTournament:
    ...
    def _create_test_strategies(self):
        ...
        for i in range(3):
            ...
            chain = StrategyChain(strategy=strategy, source_sample=i, temperature=0.7)
    ...
    async def test_basic_elimination_tournament(self):
        ...
        winner, results = await self.tournament.run_tournament(
            self.strategies, "Test context", num_comparisons=1, save_results=False
        )
    ...
    async def test_tournament_with_voting(self):
        ...
        winner, results = await self.tournament.run_tournament(
            self.strategies, "Test context", num_comparisons=2, save_results=False  # 2 comparisons per pair
        )
    ...
    async def test_round_robin_tournament(self):
        ...
        results = await self.tournament.run_round_robin_tournament(self.strategies, "Test context", num_comparisons=1)
    ...
    async def test_insufficient_strategies_error(self):
        ...
        with pytest.raises(ValueError, match="Tournament requires at least 2 strategies"):
            await self.tournament.run_tournament([self.strategies[0]], "Test context")  # Only one strategy

    def test_vote_aggregation(self):
        """Test vote aggregation logic"""
        # Test clear winner
        comparison_results = [{"preferred_strategy": 1}, {"preferred_strategy": 1}, {"preferred_strategy": 2}]
        ...
        tie_results = [{"preferred_strategy": 1}, {"preferred_strategy": 2}]
    ...
    def test_serialization(self):
        ...
        test_obj = {"strategy": self.strategies[0], "nested": {"value": 42}, "list": [1, 2, 3]}
...
class TestTournamentRunner:
    ...
    def _create_test_strategies(self):
        ...
        for i in range(3):
            strategy = GenesisStrategy(
                ...
                steps=[
                    StrategyStep(
                        action=f"Action {i+1}", prerequisites=[], outcome=f"Outcome {i+1}", risks=[], confidence=0.8
                    )
                ],
    ...
    async def test_find_best_strategy_elimination(self):
        ...
        winner, results = await self.runner.find_best_strategy(
            self.strategies, "Test context", tournament_type="elimination"
        )
    ...
    async def test_find_best_strategy_round_robin(self):
        ...
        winner, results = await self.runner.find_best_strategy(
            self.strategies, "Test context", tournament_type="round_robin"
        )
    ...
    async def test_invalid_tournament_type(self):
        ...
        with pytest.raises(ValueError, match="Unknown tournament type"):
            await self.runner.find_best_strategy(self.strategies, "Test context", tournament_type="invalid_type")
    ...
    async def test_compare_technique_strategies(self):
        ...
        results = await self.runner.compare_technique_strategies(technique_strategies, "Test context")
    ...
    async def test_tournament_with_real_strategy_structure(self):
        ...
        strategy_data = [
            {
                ...
                "steps": [
                    ...
                    (
                        "Scale Operations",
                        ["Proven demand", "Operational systems"],
                        "Sustainable growth",
                        ["Scaling challenges"],
                        0.8,
                    ),
            ...
            },
            {
                ...
                "steps": [
                    (
                        "Market Research",
                        ["Define scope", "Allocate research budget"],
                    ...
                    (
                        "Gradual Expansion",
                        ["Validated market", "Strong foundation"],
                        "Stable growth",
        ...
        for i, data in enumerate(strategy_data):
            ...
            for action, prereqs, outcome, risks, confidence in data["steps"]:
                steps.append(
                    StrategyStep(
                        action=action, prerequisites=prereqs, outcome=outcome, risks=risks, confidence=confidence
                    )
                )
        ...
        winner, results = await runner.find_best_strategy(
            strategies, "Startup strategy selection", tournament_type="elimination"
        )
