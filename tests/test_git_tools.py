...
class TestGitTools:
    ...
    async def test_git_repo_tool_llm_format(self):
        ...
        request = GitRepoRequest(
            repo_url="https://github.com/octocat/Hello-World.git", output_format="llm", max_file_size=5000
        )
        result = await git_repo_to_md_tool(None, request)
        ...
        assert len(result) > 0
        assert (
            "LLM-optimized content" in result
            or "conversion failed" in result
            or "not available" in result
            or "git2md failed" in result
        )
    ...
    async def test_local_repo_tool(self):
        ...
        request = LocalRepoRequest(repo_path=str(current_repo), output_format="markdown", max_file_size=5000)
        result = await local_repo_to_md_tool(None, request)

        assert isinstance(result, str)
        assert len(result) > 0
        assert (
            "Repository content" in result
            or "conversion failed" in result
            or "not available" in result
    ...
    async def test_local_repo_nonexistent(self):
        ...
        request = LocalRepoRequest(repo_path="/path/that/does/not/exist", output_format="markdown")
        result = await local_repo_to_md_tool(None, request)
    ...
    async def test_local_repo_not_git(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            request = LocalRepoRequest(repo_path=temp_dir, output_format="markdown")
            result = await local_repo_to_md_tool(None, request)
    ...
    async def test_git_repo_invalid_url(self):
        ...
        request = GitRepoRequest(
            repo_url="https://github.com/invalid/nonexistent-repo-12345.git", output_format="markdown"
        )
        result = await git_repo_to_md_tool(None, request)
